...
2. **Given** 解析后的会话记录，**When** 检查每条记录，**Then** 包含 display、timestamp、project、sessionId 四个字段。
...
- **FR-003**: 系统 MUST 提取每条记录的 display、timestamp、project、sessionId 字段。
- **FR-004**: 系统 MUST 支持全文搜索，匹配 display 字段内容。
- **FR-005**: 系统 MUST 支持收藏指定的 sessionId。
- **FR-006**: 系统 MUST 支持取消收藏指定的 sessionId。
...
- **SessionRecord**: 代表一条会话记录，包含 display（内容）、timestamp（时间戳）、project（项目路径）、sessionId（会话标识）。
- **FavoriteSession**: 代表收藏的会话，只需存储 sessionId 和收藏时间。
